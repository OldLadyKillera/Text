...
def find_closest_noun(tag_tokens, index):
    ...
    if index < len(tag_tokens) - 1:
        for j in range(index + 1, len(tag_tokens)):
            if tag_tokens[j][0] in string.punctuation:
                break
            if ('NN' in tag_tokens[j][1]) & (backward_distance < forward_distance) & (backward_distance <= count):
                noun = tag_tokens[j][0]
                break
            backward_distance += 1
    return noun
...
for sentence in sentence_list:
    ...
    for i in range(0, (len(tag_tokens) - 1)):
        if 'JJ' in tag_tokens[i][1]:
            if find_closest_noun(tag_tokens, i) != '':
                if i > 0:
                    if tag_tokens[i - 1][0].lower() == 'not':
                        word_frequency[('not ' + tag_tokens[i][0], find_closest_noun(tag_tokens, i))] += 1
                    else:
                        word_frequency[(tag_tokens[i][0], find_closest_noun(tag_tokens, i))] += 1
